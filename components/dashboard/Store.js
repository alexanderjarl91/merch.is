...
export default function Store() {
  ...
  return (
    <div className={styles.component_container}>
      ...
      <div>
        ...
        }}className={styles.input} type="text" placeholder={userData.store.name}></input>
      </div>
      <div>
        <label>Hlekkur á mynd</label>
        <input onChange={(e)=>{
          setLogo(e.target.value)
        }} className={styles.input} type="text" placeholder={userData.store.logo}></input>
      </div>
      <div>
      ...
      <textarea onChange={(e)=>{
        ...
        }}rows="5" className={styles.input} type="text" placeholder={userData.store.bio}></textarea>
      </div>
      <div>
        <button onClick={()=>{
          setEdit(false)
 ...
 : <>
     ...
     <p>{userData.store.bio}</p>
      ...
      <h4>Þinn hlekkur:</h4>
      <p>www.merch.is/{userData.store.name}</p>
    </div>
  );
